...
function game(){
	...
	*/
		...
		window.document.onkeydown = function(e){
			...
			switch(e.keyCode){
				case 81/*q*/:
					...
					game.hero[0].angle+=(Math.PI/72);
					break;
				case 87/*w*/:
					game.hero[0].angle-=(Math.PI/72);
					break;
				case 70/*f*/:
					game.hero[1].angle+=(Math.PI/72);
					break;
				case 71/*g*/:
					game.hero[1].angle-=(Math.PI/72);
					break;
	...
	game.draw = function(i){
		if(game.hero[i].running){
			...
			if((x<game.canvas.width) && (x>0) && (y<game.canvas.height) && (y>0)){
				var data = game.ctx.getImageData(x,y,1,1).data;
	...
	game.change_vector = function(i){
		if(game.hero[i].running){
			var x = Math.cos(game.hero[i].angle)*2;
			var y = Math.sin(game.hero[i].angle)*2;
